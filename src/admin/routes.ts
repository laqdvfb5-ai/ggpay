...
import { createTenant, getTenant, issueApiKey, revokeApiKey, upsertWebhook } from '../tenant/store.js';

export function adminRoutes():Router{
 const r=Router(),c=loadConfig();
 r.use('/admin/v1',(req,res,next)=>{if(!tokensMatch(extractAuthToken(req.header('authorization')),c.inspectToken)){res.status(401).json({error:'Unauthorized'});return;}next();});
